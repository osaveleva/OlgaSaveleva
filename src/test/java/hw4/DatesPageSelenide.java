...
public class DatesPageSelenide extends TestBaseSelenide {
    ...
    public void beforeClass() {
        ...
        datePage = open(PropertyManager.getInstance().getURL(), DatesSelenidePageObject.class);
    }
    ...
    public void checkServicePage() {
        ...
        //2. Perform login
        datePage.login(PropertyManager.getInstance().getLogin(), PropertyManager.getInstance().getPassword());

        //3. Assert User name in the left-top side of screen that user is loggined
        datePage.userName.shouldHave(text(PropertyManager.getInstance().getPageTitle()));

        // 4. Click on "Service" subcategory in the header and check that drop down contains options
        datePage.checkServiceMenu(LeftMenuItems.SERVICE.getRecord());

        //5. Open through the header menu Service -> Different Elements Page
        datePage.clickMenuItem(datePage.subCollection, SubMenuItems.DATES.getRecord());
